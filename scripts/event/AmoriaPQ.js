...
function setup(eim) {
	...
	var eim = em.newInstance("AmoriaPQ");
	
	var mf = eim.getMapFactory();
	
	var map = mf.getMap(670010200);
...
function clearPQ(eim) {
	...
	var iter = eim.getPlayers().iterator();
        ...
        while (iter.hasNext()) {
	        ...
	        player.getClient().getSession().write(org.packet.MaplePacketCreator.getClock(120));
		}
...
function playerClocks(eim, player) {
  if (player.getMap().hasTimer() == false){
	player.getClient().getSession().write(org.packet.MaplePacketCreator.getClock((Long.parseLong(eim.getProperty("entryTimestamp")) - System.currentTimeMillis()) / 1000));
