...
function setup(mapid) {
	var roomNum = (mapid % 1000) / 100; // Room Number
    var eim = em.newInstance("MCPQ" + roomNum);
	// Setup the Maps
	exitMap = em.getChannelServer().getMapFactory().getMap(980000010); // Exit
	waitingMap = em.getChannelServer().getMapFactory().getMap(mapid); // Lobby
	fieldMap = em.getChannelServer().getMapFactory().getMap(mapid + 1); // Battle Field
	fieldMap.clearAndReset(true);
	reviveMap = em.getChannelServer().getMapFactory().getMap(mapid + 2); // Resurrection
	winnerMap = em.getChannelServer().getMapFactory().getMap(mapid + 3); // Victorious
	loserMap = em.getChannelServer().getMapFactory().getMap(mapid + 4); // Defeated
...
function registerCarnivalParty(eim, carnivalParty) {
	if (eim.getProperty("red").equals("-1")) {
		...
		getCarnivalParty(eim, "red").sendClock(10);
...
function start(eim) {
	...
	eim.getMapInstance(fieldMap.getId()).addDefaultCPQMobs(leader, leader.getMapId());
...
function timeOut(eim) {
	...
	if (!eim.getProperty("started").equals("true")) {
		...
		eim.getMapInstance(fieldMap.getId()).killAllMonsters();
