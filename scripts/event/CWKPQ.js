...
function setup(eim) {
	...
	for (var i = 0; i < mapIds.length; i++) {
		var map = em.getChannelServer().getMapFactory().getMap(610030000 + mapIds[i]);
		if (map != null) {
			map.clearAndReset(true);
			// Warrior room
			if (map.getId() == 610030510) {
				for (var z = 0; z < pos_y2.length; z++) {
					var mob = MapleLifeFactory.getMonster(9400582);
			...
			// Bowman Room
				...
				for (var z = 0; z < pos_x.length; z++) {
					var mob = MapleLifeFactory.getMonster(9400594);
					eim.registerMonster(mob);
...
function playerExit(eim, player) {
	...
	var exitMap = em.getChannelServer().getMapFactory().getMap(610030010);
