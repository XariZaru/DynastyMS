...
function start() {
   if (cm.getParty() == null) {
       cm.sendOk("You must have a party to enter to fight Zakum.");
       cm.dispose();
   } else if (!cm.isLeader()) {
       cm.sendOk("The leader must speak with me before you can enter.");
   } else if (cm.partyMembersInMap() < min) {
       cm.sendOk("You must have a party of at least #b"+min+"#k people to fight Zakum.");
       cm.dispose();  
   } else {
       cm.removeFromParty(fire, cm.getPartyMembers());
       cm.sendAcceptDecline("Are you and your party ready to enter to fight Zakum? I will give you an Eye of Fire to spawn Zakum.");
   }
}

function action(m,t,s) {
    if (m == 1) {
        if (cm.getPlayerCount(map) < 1) {
            cm.warpParty(280030000);
            cm.resetMap();
            cm.gainItem(fire);
        } else {
            cm.sendOk("There is currently a party inside fighting Zakum. Please wait or change channels.");
        }
    }
    cm.dispose();
    return true;
