 ...
 */
var status, sel;
var use = [[2040826,2049100,2049000,2340000],[20,50,65,150]];
...
function start() {
    ...
    cm.sendNext("Hey, #b#h ##k, I exchange your voting points for some radical prizes."+
        " Each time you vote you receive #b1#k points, which can occur every 6 hours from each of our"+
    " voting links.");
}
    ...
    if (status == 0) {
        ...
        if (s == 0) {
            cm.sendGetNumber("How many voting points will you spend on NX? Each"+
                " point is worth #b"+nx+" pre-paid NX#k. You have #e"+cm.getPlayer().getVp()+"#n"+
                " vote points.",1,1,100);
        } else if (s == 1) {
        ...
        if (sel == 0) {
            if (cm.getPlayer().getVp() < s) {
                ...
                cm.sendOk("You have gained #b"+nx * s+"#k prepaid NX in exchange for #b"+s+"#k voting points");
                cm.getPlayer().getCashShop().gainCash(4, nx * s);
