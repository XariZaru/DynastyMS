...
" arduous tasks that would cripple some of our most experienced men.");
    ...
    " cause behind them.");
        ...
        if (cm.getLevel() < 8) {
            ...
            cm.talkGuide("Taeng wishes for us to train a little longer. He said he'd compensate us."+
        ...
        if (!cm.haveItem(cap, 20) || !cm.haveItem(leaf, 25)) {
            cm.sendOk("Where are those items I told you to get? You need proof!\r\n\r\n"+("#B"+(cm.itemQuantity(cap)/20 * 100))+"#  "+(cm.itemQuantity(cap)/20 * 100
        )+"% #i"+cap+"# collected\r\n"+("#B"+(cm.itemQuantity(leaf)/25 * 100)+"#  "+(cm.itemQuantity(leaf)/25 * 100
        )+"% #i"+leaf+"# collected")+"");  
            cm.dispose();
        } else {
