        ...
        if(cm.getLevel() >= 30 && cm.getJobId() == 100) {
            if(cm.haveItem(4031008)) {
                if(!cm.haveItem(4031013)) { // Check to see if they have any dark marbles already.
                    if (status == 0) {
                        cm.sendNext("Hmm. That's definitely a letter from #p1022000#. So you came here to try the test for your job advancement? Okay, I will explain the test to you then. Don't worry, it's not difficult.");
                    } else if(status == 1) {
                ...
                cm.sendOk("You want to become a much stronger warrior than you already are? I'll take care of that, you seem to be more than qualified. Go talk to #b#p1022000##k in #m102000000# first, then come back to see me.");
                cm.dispose();
            }
        } else if(cm.getLevel() < 30 && cm.getJobId() == 100) {
            cm.sendOk("You want to become a much stronger warrior? I'll take care of that, but you seem too weak. Keep training, become more powerful and then come back to see me.");
            cm.dispose();
        } else if(cm.getJobId() > 100 && cm.getJobId() < 200) {
