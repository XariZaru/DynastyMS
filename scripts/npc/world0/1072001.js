...
/* Warrior Job Instructor
	Map Name (Map ID)
	Used to warp to the test for warrior second job advancement.
 */
        ...
        if(cm.getLevel() >= 30 && cm.getJobId() == 200) {
            if(cm.haveItem(4031009)) {
                if(!cm.haveItem(4031013)) { // Check to see if they have any dark marbles already.
                    if (status == 0) {
                        cm.sendNext("Hmm. That's definitely a letter from #p1032001#. So you came here to try the test for your job advancement? Okay, I will explain the test to you then. Don't worry, it's not difficult.");
                    } else if(status == 1) {
                ...
                cm.sendOk("You want to become a more powerful magician than you already are? I'll take care of that, you seem to be more than qualified. Go talk to #b#p1032001##k in #m101000000# first, then come back to see me.");
                cm.dispose();
            }
        } else if(cm.getLevel() < 30 && cm.getJobId() == 200) {
            cm.sendOk("You want to become a more powerful magician? I'll take care of that, but you seem too weak. Keep training, become more powerful and then come back to see me.");
            cm.dispose();
        } else if(cm.getJobId() > 200 && cm.getJobId() < 300) {
            cm.sendOk("Hmm... It was you who passed my test the other day! Have you become more powerful? Good! Now keep training to grow even more powerful.");
            cm.dispose();
        } else {
            cm.dispose();
        }
