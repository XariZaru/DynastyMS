...
/* Thief Job Instructor
	Map Name (Map ID)
	Used to warp to the test for thief second job advancement.
 */
        ...
        if(cm.getLevel() >= 30 && cm.getJobId() == 400) {
            if(cm.haveItem(4031011)) {
                if(!cm.haveItem(4031013)) { // Check to see if they have any dark marbles already.
                    if (status == 0) {
                        cm.sendNext("Hmm. That's definitely a letter from #p1052001#. So you came here to try the test for your job advancement? Okay, I will explain the test to you then. Don't worry, it's not difficult.");
                    } else if(status == 1) {
                ...
                cm.sendOk("You want to become a much stronger warrior than you already are? I'll take care of that, you seem to be more than qualified. Go talk to #b#p1052001##k in #m103000000# first, then come back to see me.");
                cm.dispose();
            }
        } else if(cm.getLevel() < 30 && cm.getJobId() == 400) {
            cm.sendOk("You want to become a better thief? I'll take care of that, but you seem too weak. Keep training, become stronger and then come back to see me.");
            cm.dispose();
        } else if(cm.getJobId() > 400 && cm.getJobId() < 500) {
            cm.sendOk("Hmm... It was you who passed my test the other day! Have you become a better thief? Good! Now keep training to grow even stronger.");
            cm.dispose();
        } else {
            cm.dispose();
        }
