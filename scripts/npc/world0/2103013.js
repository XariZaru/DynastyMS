var text = [
            ["You first have to form a party, regardless of whether or not it's only you or with 1-2 other people. Once you've gotten a party, you may enter" +
            		" by speaking directly to me. The level limits for this dungeon is levels #b40-70#k."],
            		
            ["The rules of this dungeon? #eHa!#n As if there are any! There is no rule to this place, no limit. Just go in and kill as many of those bastards" +
            		" as possible. The more you kill, the higher the rewards the #bPharaoh#k will grant to you. If you happen to die, however, we won't take" +
            		" kindly to those who are weak and will fail to reward you for your pitiful and incomplete work.", "At the end of every dungeon, you will" +
            				" be rewarded for your performance. The higher it is, the more rewards you'll get along with the Pharaoh's gift, which may be spent" +
            				" at one of our shops nearby or at the global Free Market place."],	
            ["Whatever"]];
...
function start() {
	if (cm.getParty() == null)
		cm.sendNext("Haha! What brings you to the sands of this fine, fine country? Come, come. Tell me, what brings" +
				" such fine specimens to the Pyramid's royal court? Oh, you're a #b" + (cm.getJobId() >= 1000 && cm.getJobId() < 2000 ? "royal knight from the Imperial" +
						" Empire" : cm.getJobId() < 900 ? "warrior from the Explorer tribe" : "lonely warrior whose origin I cannot ascertain at this time") + "#k. " +
								"Anywho, there is much to talk about regardless of why you are here!");
	if (cm.getPlayer().getDynastyPQ() != null)
		cm.sendYesNo("What's this? Come to give up on the dungeon already? The Pharaoh doesn't like those who " +
				"leave, and thus you'll get no rewards for participating!");
	else  {
		cm.warp(926010000);
		cm.startDynastyPyramid(926010000);
		cm.dispose();
	}
	status = -1;
}
...
function action(m,t,s) {
	...
	if (status == 0) {
		if (cm.getParty() == null)
			cm.sendSimple("So, young man, what would you like to learn about this great, great Pyramid?", "How do I play in this dungeon?", "What are the rules?",
					"Can you give me more details about what to do in there?");
		else {
			cm.getPlayer().getDynastyPQ().endPQ();
			cm.dispose();
		}
	} else if (status == 1) {
		if (s != -1) {
		...
		if (cm.getParty() == null) {
			if (index < text[chosenText].length) {
				cm.sendNext(text[chosenText][index]);
				status = 0;
				index++;
			}
		}
