...
function start(){
    if (cm.getMapId()==103000000) {
        switch(cm.getKerningCBDPlaneStatus()){ //NOBOAT(0), TOOLATE(1), CANBOARDBOAT(2)
            case 0:
                cm.sendOk("The plane to Singapore has already taken off, please be patient and wait for next one.");
                cm.dispose();
                break;
            case 1:
                cm.sendOk("The plane to Singapore is ready to take off, please be patient and wait for next one.");
                cm.dispose();
                break;
            case 2:
