...
public final class CashOperationHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (action == 0x03 || action == 0x1E) {
            ...
            //if (item.getCashId() >= 10001168 && item.getCashId() <= 10001176 || item.getCashId() == 5211048) { 
//                return; 
//            } 
            if (action == 0x03) { // Item
            	if (item.getItemId() >= 5211000 && item.getItemId() <= 5211049 || item.getItemId() >= 5360000
            			&& item.getItemId() <= 5360014 || item.getItemId() == 5220000 || item.getItemId() == 5360042 ||
            			cannotBuy(item)) {
            		denyBuy(c);
            		return;
            	}
                ...
                for (Item item2 : cashPackage) {
                    cs.addToInventory(item2);
                }
    ...
    public boolean cannotBuy(Item item) {
    	try {
	    	...
	    	PreparedStatement ps = con.prepareStatement("SELECT * FROM blocked_cs_items WHERE itemid = ? LIMIT 1");
