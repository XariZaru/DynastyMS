...
public class HPQ extends PartyQuest {
        ...
        if (eim != null) {
            ...
            if (stage <= 6) { //if it exceeds 6, map crash i.e. your client gets a INT min value (-2147blabla) error.
                ...
                if (stage == 6) { //trigger fullmoon only when stage is 6, no need to do it otherwise.
                    fullMoon.runEvents(eim.getPartyLeaderChar().getClient());
                }
            }
        }
    }
    ...
    public boolean calcAndUpdateRank(MapleCharacter chr, final MapleCharacter.PQRankRecord pqRankRecord) {
        ...
        String areaInfo = "cmp=" + completed + ";try=" + tries
                + ";have=" + (haveItem ? 1 : 0) + ";rank=" + rank_;

        //update rank in db and area info (quest info UI)
        updateRank(chr,
                !rank.equalsIgnoreCase(rank_) ? null : rank_, //update ranks only if it's diff from the one in db
                areaInfo,
                fastestTime,
                fastestDate);
    ...
    public static void giveRandomizedRiceCakeHat(MapleCharacter chr) {
        ...
        while (amount == 0) { //cuz i want the player to have ONE stat on the hat :D
            ...
            //0 - 2
            amount = (byte) MapleItemInformationProvider.getRandStat((short) 1, 2);
        }

        int hatId = 1002798;
        Equip hat = (Equip) MapleItemInformationProvider.getInstance().getEquipById(hatId);
        hat.setStat(stats[statIndex], amount);

        MapleInventoryManipulator.addFromDrop(chr.getClient(), hat, false);
        chr.getClient().announce(MaplePacketCreator.getShowItemGain(hatId, (short) 1, true));
    }
